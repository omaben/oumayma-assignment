...
export function MainNav({ color = 'evident', items = [], searchParams }: MainNavProps): React.JSX.Element {
...
function DropdownItem({
  ...
  items?.map((childItem: NavItemConfig) => {
    if (
      user?.role
  ...
  if (childItemsData.length > 0) {
    return (
      <Dropdown>
        ...
        >
          {renderDropdownItems({ pathname, items: childItemsData })}
        </DropdownPopover>
      </Dropdown>
    );
  }
