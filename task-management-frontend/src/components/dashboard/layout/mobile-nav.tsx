  ...
  const children = items.reduce((acc: React.ReactNode[], curr: NavItemConfig): React.ReactNode[] => {
    acc.push(
      <Stack component="li" key={curr.key} spacing={1.5}>
        ...
        <div>{pathname && renderNavItems({ depth: 0, items: curr.items, onClose, pathname })}</div>
      </Stack>
    );
...
function renderNavItems({
  ...
  const children = items.reduce((acc: React.ReactNode[], curr: NavItemConfig): React.ReactNode[] => {
    ...
    const forceOpen = childItems
      ...
      : false;
    const childItemsData: NavItemConfig[] | undefined = [];

    childItems?.map((childItem: NavItemConfig) => {
      if (
        user?.role) {

        childItemsData.push(childItem)
      }
    });

    if (
      !item.userMenuPermission
     
    ) {
      acc.push(
        <NavItem depth={depth} forceOpen={forceOpen} key={key} pathname={pathname} {...item}>
          {childItemsData ? renderNavItems({ depth: depth + 1, pathname, items: childItemsData }) : null}
        </NavItem>
      );
    }
